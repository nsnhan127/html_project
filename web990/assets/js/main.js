...
if ($("#userBox").length > 0){
  document.getElementById("userBox").addEventListener('click', function (event) {
    event.stopPropagation();
  });
}

if ($("#ring-bell").length > 0){
  document.getElementById("ring-bell").addEventListener('click', function (event) {
    event.stopPropagation();
  });
}
  ...
  $(window).scroll(function() {
    ...
    if ($('html').hasClass("isDesktop") || $('html').hasClass("isTablet")){
      if (windowScroll + div_height > contactBox - 100){
        ...
        $('#sidebar').css({top: (windowScroll + div_height - contactBox + padding) * -1})
      }
...
function substicky(el) {
  if (el.length) {
    ...
    $(window).scroll(function() {
      ...
      if ($(window).scrollTop() > scrollHeight - h_footer - h_sec5 - h_el -300) {
